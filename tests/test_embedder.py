...
def test_cache_key_includes_model(embedder_env):
    ...
    assert a._get_text_hash("same text", "document") != b._get_text_hash("same text", "document")


def test_cache_key_includes_input_type(embedder_env):
    """Query and document embeddings of the same text must not share cache entries."""
    e = _make(embedder_env)
    assert e._get_text_hash("same text", "document") != e._get_text_hash("same text", "query")

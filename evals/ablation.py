...
os.environ.update({"EMBEDDING_PROVIDER": "openrouter", "OPENROUTER_EMBED_MODEL": "qwen/qwen3-embedding-8b",
...
def rrf(*lists, k=60):
    ...
    for L in lists:
        for r, x in enumerate(L):
            sc[x.content] = sc.get(x.content, 0) + 1/(k+r+1); obj[x.content] = x
...
async def main():
    ...
    emb = make_embedder(); rr = CohereReranker()
    gold_key = {}
    async with store.pool.acquire() as conn:
        for g in golden:
            row = await conn.fetchrow("SELECT content FROM notes WHERE path=$1 AND chunk_index=$2", g["path"], g["chunk"])
            gold_key[(g["path"], g["chunk"])] = _norm(row["content"]) if row else None

    def rank_of(g, ordered):
        key = gold_key[(g["path"], g["chunk"])]
        return next((i+1 for i, r in enumerate(ordered) if r.path == g["path"] and _norm(r.content) == key), None)

    cfgs = {"dense": [], "dense+rerank": [], "dense FUSED rerank(RRF)": [], "full(hybrid+rerank)": []}
    for g in golden:
